%% Title:    THA 4, Part C - Distance, Joint Limits, & Rotation Constraints with Virtual Wall
...
% Set initial y location from virutal wall
y_location = inf;
...
for z = 1:p_rows/4
    ...
    if (IsGood == true)
        ...
        while (error_w > eps_w && error_v > eps_v && abs(y_location) > 4 && IsGood == true)
            ...
            f1 = [-t_hat    , eye(3,3)  ;
                zeros(3,3), zeros(3,3)];
            h1 = space_j;
            J1 = f1*h1;    % Jacobian for First Constraint from Linearized Equations
            ...
            for j = 1:n
                A3_alpha = (i*2*pi)/n;
                A3_beta = (i*2*pi)/m;
                A3(i,:) = [0 , 0 , 0 , cos(A3_alpha)*cos(A3_beta) , cos(A3_alpha)*sin(A3_beta) , sin(A3_alpha)];
            end
            ...
            % /////////////////////////////////////////////////////////////
            % DEVELOP VIRTUAL WALL
            % Plane is in the x-z plane

            % Define vector perpindicular to the plane (y-direction)
            dt = [0 , 1 , 0];  

            % A matrix from Ming Li Paper
            A4 = [ -dt , 0 , 0 , 0];

            % Set y-distance of virtual wall
            offset = 600;

            % Plot the Virutal Wall
            [x_mesh, z_mesh] = meshgrid(-2000:4000:2000); % Generate x and y data
            y_mesh = zeros(size(x_mesh, 1)) + offset; % Generate z data
            surf(x_mesh, y_mesh, z_mesh,'FaceColor','c') % Plot the surface
            alpha(0.1)
            hold on;

            % Set Virtual Wall
            y_location = t(2)-offset;
            del_VF = [0;y_location;0;0;0;0];
            b4 = - A4*del_VF;
            ...
            A = [LhsConst1;LhsConst2;LhsConst3;LhsConst4];
            b = [RhsConst1;RhsConst2;RhsConst3;RhsConst4];
            ...
            % Test if wall was hit to stop robot from moving
            if (y_location < 40)
                theta = theta_last;
                disp("I Hit the Wall!")
                IsGood = false;
            ...
            % Set for virtual wall constraint
            theta_last = theta;

        end % while loop end

        %Replace figure if another iteration if comming
        if (error_w > eps_w && error_v > eps_v && y_location > 40)
            clf(InFigure);
        end
