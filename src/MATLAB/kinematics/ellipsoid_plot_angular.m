function []=ellipsoid_plot_angular(Jacobian, T,figure_name)

% Turn off Warnings
warning('off','all')

% Grab Eigenvectors and EigenValues
[vectors,lambdas] = GetJacobianEigs(Jacobian,"angular");
...
figure(figure_name);
plot3(xy_vec(1,:),xy_vec(2,:),xy_vec(3,:),'y')
line([xc xy_vec(1,1)],[yc xy_vec(2,1)],[zc xy_vec(3,1)],'color', 'y', 'linewidth', 4);
hold on;
...
figure(figure_name);
plot3(xz_vec(1,:),xz_vec(2,:),xz_vec(3,:),'m')
line([xc xz_vec(1,1)],[yc xz_vec(2,1)],[zc xz_vec(3,1)],'color', 'm', 'linewidth', 4);
hold on;
...
figure(figure_name);
plot3(yz_vec(1,:),yz_vec(2,:),yz_vec(3,:),'c')
line([xc yz_vec(1,1)],[yc yz_vec(2,1)],[zc yz_vec(3,1)],'color', 'c', 'linewidth', 4);
hold on;
