%% Title:    THA 2, Body Frame Foward Kinematics
...
function FK=FK_body(s_hat, q, h, M, theta, PlotMe)
...
% Determine if the user wants to plot
if (PlotMe)
    axis square;
...
% Get Body Frame Screws
B_Screws = GetScrewVector(s_hat,q,h);

% Loop Through Joints
for i = 1:cols

    % Save the Foward Kinematics from Previous Joint
    FK_prev = FK;

    % Post-Multiple Forward Kinematics by the Screw Exponential
    FK = FK * GetScrewExp(B_Screws(:,i),theta(i));

    % Plotting Links In Between Joints
    % Translation component between current joint and body frame
    Cur_Trans_TF = [1 0 0 q(1,i);
                    0 1 0 q(2,i);
                    0 0 1 q(3,i);
                    0 0 0 1];

    % Transform of the Current Joint to the body Frame
...
if (PlotMe)
    ...
    axis([-3000 3000 -3000 3000 -2000 3500]);
    grid on;
    %title('Body Frame Foward Kinematics');
    xlabel('x axis [mm]');
    ylabel('y axis [mm]');
    zlabel('z axis [mm]');
