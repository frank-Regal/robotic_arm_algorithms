%% Title:    THA 2, DLS Inverse Kinematics
...
function theta=DLS_inverse_kinematics(Tsd,StartThetas, InFigure)
...
while (error_w > eps_w && error_v > eps_v)
    fprintf("\n\n===================================================\n");
    ...
    % Get Eigenvalues for linear nad angular ellipsoids for current
    % configuration
    [~,vals_w] = GetJacobianEigs(body_j,"angular");
    [~,vals_v] = GetJacobianEigs(body_j,"linear");

    % Get Isotropy
    isotropy_w = J_isotropy(vals_w);
    isotropy_v = J_isotropy(vals_v);
    
    % Get Jacobian size
    [rows,cols] = size(body_j);
    
    % Check if close to singularity
    if (isotropy_w > 5 || isotropy_v > 5) 
        % damped least squares
        k = 0.5;
        J_dls = transpose(body_j) * inv(body_j*transpose(body_j) + (k^2)*eye(6));
        body_j_inverse = J_dls;
